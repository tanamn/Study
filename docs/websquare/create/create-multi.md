...
Submission을 이용해 Spring·MyBatis에서 일괄 처리하는 방법을 학습한다.
...
Grid의 고객명·생년월일·연락처·주소·직업 컬럼은 사용자가 값을 입력할 수 있도록 편집 가능 상태로 설정한다.
...
`addRow()`는 DataList에 신규 행을 추가한다. Grid는 DataList와 바인딩되어 있으므로 추가된 행이 화면에도 표시된다.

프로젝트의 WebSquare 버전과 공통 라이브러리에 따라 행 추가 API가 감싸져 있을 수 있으므로 기존 입력 Grid 화면도 함께 확인한다.
...
Grid에서 선택한 행 번호를 가져온 뒤 실제 데이터를 보유한 DataList에서 삭제한다.
...
행 선택 API와 삭제 방식은 프로젝트별 Grid 설정에 따라 다를 수 있다.
...
검증 오류 메시지에는 몇 번째 행에서 오류가 발생했는지 포함하는 것이 좋다.

```text
3행의 연락처 형식을 확인해주세요.
```

`setFocusedCell()`의 인자 형식은 프로젝트에서 사용하는 WebSquare 버전에 따라 다를 수 있으므로 실제 Grid API를 확인해야 한다.

### 7. Grid 내부 중복 검사

동일한 연락처가 여러 행에 입력됐는지 등록 전에 확인할 수 있다.
...
화면의 중복 검사는 사용자 실수를 빠르게 알려주기 위한 것이다. 최종 중복 여부는 서버와 DB에서도 검사해야 한다.
...
WebSquare Submission이 만드는 실제 JSON 구조는 프로젝트의 전송 설정과 공통 통신 모듈에 따라 달라질 수 있다. 서버 개발 전에 브라우저 개발자 도구의 Network 탭에서 요청 본문을 확인하고 Controller의 DTO 구조를 맞춰야 한다.

### 10. Spring Controller에서 `List<DTO>` 수신

요청 본문이 JSON 배열이라면 Controller에서 리스트로 받을 수 있다.
...
각 요소의 `@NotBlank`, `@Pattern` 등을 검사하려면 리스트의 요소 타입에 `@Valid`를 적용한다.

```java
List<@Valid CustomerCreateDto> customers
```

프로젝트의 Spring 및 Bean Validation 버전에 따라 메서드 파라미터 검증을 위한 추가 설정이나 `@Validated`가 필요할 수 있다.

### 11. 요청 Wrapper를 사용하는 방식

프로젝트에서 요청 본문을 객체 형태로 통일한다면 Wrapper DTO를 사용할 수 있다.
...
JSON 배열 방식과 Wrapper 방식 중 무엇을 사용할지는 프로젝트의 API 표준에 맞춰 결정한다.
...
`@Transactional`이 적용되어 있으므로 반복 처리 중 한 건이라도 예외가 발생하면 앞서 등록한 행도 함께 롤백된다.
...
이 방식은 이해하기 쉽지만 고객 수만큼 SQL을 실행한다. 수백·수천 건을 처리한다면 MyBatis Batch Executor나 별도의 대량등록 방식을 검토해야 한다.
...
고객 마스터처럼 정합성이 중요한 데이터는 일반적으로 전체 롤백 방식이 이해하기 쉽고 안전하다.

부분 성공을 적용한다면 각 행을 독립 트랜잭션으로 처리하고 결과에 행별 상태와 오류 사유를 포함해야 한다.
...
부분 성공 방식은 구현과 재처리 로직이 복잡해지므로 업무 요구사항을 먼저 확정해야 한다.
...
등록 성공 후 입력 DataList를 비울지는 화면 정책에 따라 결정한다. 사용자가 등록 결과를 확인해야 한다면 즉시 삭제하지 않고 성공 상태를 표시할 수도 있다.

### 16. 대량 데이터 처리 시 주의사항

- 화면에서 한 번에 입력 가능한 최대 행 수를 제한한다.
- 중복 클릭을 막기 위해 요청 중 등록 버튼을 비활성화한다.
- 수백 건 이상은 일정 크기로 나누어 처리하는 방식을 검토한다.
- 서버에서 요청 건수 제한을 다시 검사한다.
- DB Unique 제약조건으로 최종 중복을 방지한다.
- 실패한 행과 오류 사유를 사용자가 확인할 수 있게 한다.
- 개인정보가 서버 로그에 그대로 출력되지 않도록 주의한다.

서버에서도 최대 건수를 검사할 수 있다.
